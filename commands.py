...
def cmd_commands(context):
    ...
    print('* preskumaj - vypise popis veci')
...
# def cmd_investigate(item, context):
#    if not item:
#        print("neviem co mam prezkumat")
#
#    item_room = get_item_by_name(item, context['room']['items'])
#    item_back = get_item_by_name(item, context['backpack']['items'])
#    if not item_room and not item_back:
#        print("item sa nikde nenachadza")
#    elif item_room != None:
#        print(item_room['description'])
#    else:
#        print(item_back['description'])
...
def cmd_examine_item(line :str, context: dict):
    ...
    #item not found
    if item is None:
        print(f"Predmet {item} tu nikde nevidim")
        return
